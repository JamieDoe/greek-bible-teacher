...
/**
 * Lays days out as week columns (Monday first) and assigns each a level 0–3 (the design's four lapis steps): 0 for no
...
export function heatmapWeeks(
  ...
  const level = (n: number) => (n <= 0 || max === 0 ? 0 : Math.min(3, Math.ceil((3 * n) / max)));
